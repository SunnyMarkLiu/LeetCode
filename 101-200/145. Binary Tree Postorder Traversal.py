...
class Solution:
    ...
    我们先将根结点压入栈，然后定义一个辅助结点head，while循环的条件是栈不为空，在循环中，
    首先将栈顶结点 top 取出来，如果栈顶结点没有左右子结点，或者其左子结点是head，或者其右子结点是head的情况下,
    我们将栈顶结点值加入结果res中，并将栈顶元素移出栈，然后将head指向栈顶元素；
    否则的话就看如果右子结点不为空，将其加入栈，再看左子结点不为空的话，就加入栈，注意这里先右后左的顺序是因为栈的后入先出的特点，
    可以使得左子结点先被处理。
    下面来看为什么是这三个条件呢？
    首先如果栈顶元素没有左右子结点的话，说明其是叶结点，而且我们的入栈顺序保证了左子结点先被处理，
    所以此时的结点值就可以直接加入结果res了，然后移出栈，将 head 指向这个叶结点，
    **这样的话 head 每次就是指向前一个处理过并且加入结果res的结点**，那么如果栈顶结点的左子结点或者右子结点是head的话，
    ...
    def postorderTraversal(self, root):
        ...
        while len(stack) > 0:
            ...
            if (not top.left and not top.right) or (top.left == head) or (top.right == head):
                # top 结果加入到 result
                result.append(top.val)
                # 将处理后的结点移除栈，同时 head 指向已经处理后的结点
                head = stack.pop()
            else:  # 存在左结点或右结点
